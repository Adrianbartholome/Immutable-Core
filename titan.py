...
class WeaverManager:
    ...
    def weave(self, new_hologram_id, new_text, keywords, depth=5):
        ...
        # 1. Fetch anchors (using our new logic that skips keyword requirements)
        ...
        try:
            ...
            # Clean up potential markdown wrappers from AI
            ...
            # THE RECONCILER: Handle both list and dict AI responses
            # We must ensure the keys generated by the loop match the keys in valid_candidates
            if isinstance(data, list):
                for i, item in enumerate(data):
                    key = f"CANDIDATE_{i+1}"
                    if key in valid_candidates and item.get("resonance"):
                        self.create_link(new_hologram_id, valid_candidates[key], item)
                        synapses_created += 1
            elif isinstance(data, dict):
                for key, val in data.items():
                    # Strip any potential whitespace or AI-added text from the key
                    clean_key = key.strip().upper().replace(" ", "_")
                    if clean_key in valid_candidates and val.get("resonance"):
                        self.create_link(new_hologram_id, valid_candidates[clean_key], val)
                        synapses_created += 1
            
            log(f"WEAVER: Integration Complete. {synapses_created} synapses woven.")
            return synapses_created # This is the integer the HUD needs!
